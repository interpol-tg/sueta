...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, f"Привет, {message.from_user.first_name}. Выберите опцию:",
                     reply_markup=markup)  # теперь бот обращается по имени
...
def get_podcast_links(message):
    url = message.text
    validations = validators.url(url)
    if validations:
...
def send_my_podcasts_keyboard(message, edit=False):
    chat_id = str(message.chat.id)
    if chat_id in user_notifications:
        markup = types.InlineKeyboardMarkup()
        ...
        if edit:
            bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id, text='Ваши подкасты:',
                                  reply_markup=markup)
        else:
...
def toggle_notification(call):
    ...
    send_my_podcasts_keyboard(call.message, edit=True)
...
def add_podcast_to_list(message):
    ...
    initial_episode_count = len(feed.entries)
    podcast_info = {'title': podcast_title, 'url': url, 'enabled': True, 'episode_count': initial_episode_count}
...
def check_new_episodes():
    for chat_id, podcasts in user_notifications.items():
        for podcast in podcasts:
            if not podcast['enabled']:
                continue

            feed = feedparser.parse(podcast['url'])
            new_episode_count = len(feed.entries)

            if new_episode_count > podcast['episode_count']:
                new_episodes = new_episode_count - podcast['episode_count']
                bot.send_message(int(chat_id),
                                 f"В подкасте '{podcast['title']}' доступно {new_episodes} новых эпизодов.")
                podcast['episode_count'] = new_episode_count
                save_notifications()

    Timer(43200, check_new_episodes).start()
